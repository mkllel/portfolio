...
add_javascript('<script src="'.$board_skin_url.'/jquery.lettering-0.6.min.js"></script>', 2);
?>
...
<div class="papercss" style="width:<?php echo $width; ?>">
	<header>
		<h2 id="bo_v_title">
	...
	<section id="bo_v_atc">
		...
		<?php
			...
			for($i = 0; $i < count($str_array); $i++){
				$cnt = mb_strlen($str_array[$i], 'utf-8');
			...
			$cnt =  mb_strlen($content, 'utf-8');
			$pages = ($cnt / 200);
			for($i=0; $i<$pages; $i++){
				$contents[$i] = mb_substr($content, ($i * 200), 200, 'utf-8');
			}
			// fill space end of the paper
			$str = $contents[($pages)];
...
<script>
	...
	<section id="bo_v_file">
		...
		for ($i=0; $i<count($view['file']); $i++) {
			if (isset($view['file'][$i]['source']) && $view['file'][$i]['source'] && !$view['file'][$i]['view']) {
				$str1 .= "<input type=\"hidden\" name=\"sources[]\" value=". $view['file'][$i]['source']. ">";
				$str2 .= "<input type=\"hidden\" name=\"hrefs[]\" value=". $view['file'][$i]['file']. ">";
		...
		<div class='download'>
			<form method='post' action="<?php echo $board_skin_url; ?>/download.php">
				<?php
				echo $str1. $str2;
				?>

				<input type="hidden" name="dir" value="<?php echo G5_DATA_PATH. "/file/". $bo_table. "/"; ?>">
				<input type="hidden" name="count" value="<?php echo count($view['file']); ?>">
				<input type="hidden" name="id" value="<?php echo $wr_id; ?>">
				<button type='submit'> Zip 다운로드 </button>
			</form>
		</div>
